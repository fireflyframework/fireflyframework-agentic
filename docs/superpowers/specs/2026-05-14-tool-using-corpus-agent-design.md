...
- `fireflyframework_agentic/rag/retrieval/_code_eval.py` (new, private)
...
- Tools available on day one: `knowledge_search`, `sql_query`, `inspect_table`, `code_eval`.
...
class ReasoningAnswerAgent:
    def __init__(
        ...
        *,
        ...
        self._code_eval = _build_code_eval()
        ...
        self._agent = FireflyAgent(
            ...
            tools=[self._knowledge_search, self._sql_query, self._inspect_table, self._code_eval],
            auto_register=False,
        )
...
**`code_eval(source: str, data: dict[str, Any] | None = None) → str`**
- Restricted Python execution sandbox. Full spec in Section 6.

### 6. `code_eval` sandbox (`_code_eval.py`)
...
**AST-enforced denylist** (validated by walking the parsed AST before execution; any violation returns `"code_eval error: <reason>"`):
...
**Resource limits:**
- Wall-clock timeout: 5s default, enforced by running the body in a worker thread with a join timeout. Pragmatic, not adversarially safe — we trust our own model, not an attacker. On timeout returns `"code_eval timeout after 5s"`.
...
**Dependencies:** `numpy` and `pandas` are added to a new optional extra `reasoning-eval` in `pyproject.toml`. `_code_eval.py` lazy-imports on first call; a missing import raises a clean `RuntimeError("install fireflyframework-agentic[reasoning-eval] to use code_eval")` that `answer()` surfaces back into the agent as a tool error so the model can stop calling it.
...
5. Strategy rules:
   ...
   - For numeric answers, verify with `code_eval` over the returned rows when the calculation is non-trivial (weighted means, growth rates, stdev, CV, ratios).
...
1. **`test_reasoning_answerer.py`** — `ReasoningAnswerAgent` with a stubbed pydantic-ai model. Replay a pre-baked tool-call sequence. Cases:
   ...
   - Mixed path: knowledge_search → sql_query → code_eval → Answer.
...
- `methodology.md` — defines "Operating Efficiency" = revenue per headcount. Drives the joint RAG+SQL+code_eval path.
...
|---|----------|------|-----------|
| 1 | "What's the YoY revenue growth rate per business unit from 2023 to 2024?" | sql_query → code_eval | Each BU's growth rate within 0.1pp; trace has ≥1 `sql_query` followed by `code_eval` whose `data` derives from the SQL rows |
| 2 | "What's the weighted average price across products, weighted by units sold?" | sql_query → code_eval (`sum(p*u)/sum(u)`) | Numeric within 0.01; `code_eval.source` contains the weighted-mean formula |
| 3 | "For Q4 2024 revenue, treat blank cells as 0 — what's the mean per region and the standard deviation?" | sql_query with COALESCE → code_eval using `statistics.stdev` | Both stats within 0.01; `code_eval.source` references `statistics` |
| 4 | "What's the coefficient of variation of monthly headcount per BU, and rank BUs most-stable to least-stable?" | sql_query → code_eval (CV per BU, sort) | Ranking matches ground truth exactly |
| 5 | "What's the Operating Efficiency for each BU in 2024 Q3?" | knowledge_search → sql_query → code_eval | Each BU within 0.5; trace has `knowledge_search` BEFORE `sql_query`; `Answer.text` cites the methodology chunk_id |
...
3. Real tool closures run against the real DB and `_code_eval` sandbox.
...
- Trace contains ≥1 `sql_query` and ≥1 `code_eval`.
- `code_eval.source` references variable names populated from a prior `sql_query` observation (regex grep).
...
5. Run the final `code_eval` step and assert its returned value matches.
...
- **Span** `firefly.rag.reasoning.answer` (replaces `firefly.rag.answer` when `strategy=reasoning`). Attributes: `tool_calls_total`, `tool_calls_by_name.knowledge_search`, `tool_calls_by_name.sql_query`, `tool_calls_by_name.inspect_table`, `tool_calls_by_name.code_eval`, `n_messages`, `terminal_state ∈ {answered, no_info, tool_limit, llm_limit, timeout, error}`.
...
1. `_code_eval.py` + sandbox unit tests. Standalone — no other deps.
2. Tool closures (`knowledge_search`, `sql_query`, `inspect_table`, `code_eval`) over `_LoopContext` + per-tool unit tests with stubbed `corpus_agent` / `structured_retriever`.

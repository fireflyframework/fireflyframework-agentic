...
async def ingest_one(
    *,
    ...
    def _record_terminal(status: str, n_chunks: int) -> IngestionResult:
        elapsed_ms = (time.perf_counter() - document_start) * 1000.0
        ingest_documents.add(1, {"status": status, "mime_type": mime_type})
        ingest_document_duration.record(elapsed_ms, {"status": status, "mime_type": mime_type})
        if bytes_seen:
            ingest_bytes.add(bytes_seen, {"status": status, "mime_type": mime_type})
        if n_chunks:
            ingest_chunks.add(n_chunks, {"status": status, "mime_type": mime_type})
    ...
    # Outer document span: histogram is recorded by ``_record_terminal`` at
    # exit so the status/mime_type labels reflect the actual terminal state.
    async with timed_span(
        ...
        try:
            async with timed_span(
                "rag.ingest.load",
                histogram=ingest_stage_duration,
                attributes={"doc_id": doc_id, "source_path": source_path},
                metric_labels={"stage": "load", "status": "success"},
            ):
            ...
            return _record_terminal("load_failed", 0)
        ...
        async with timed_span(
            ...
            metric_labels={"stage": "chunk", "status": "success"},
        ) as chunk_span:
            ...
            chunk_span.set_attribute("firefly.rag.chunk_count", len(stored_chunks))
        ...
        try:
            async with timed_span(
                ...
                histogram=ingest_stage_duration,
                attributes={"doc_id": doc_id},
                metric_labels={"stage": "reset", "status": "success"},
            ) as reset_span:
                ...
                reset_span.set_attribute("firefly.rag.prior_chunks_deleted", len(prior_ids))
            ...
            if not stored_chunks:
                ...
                async with timed_span(
                    ...
                    histogram=ingest_stage_duration,
                    attributes={"doc_id": doc_id},
                    metric_labels={"stage": "ledger", "status": "success"},
                ):
                    await ledger.upsert(doc_id, source_path, content_hash, status="success")
                return _record_terminal("success", 0)
            ...
            async with timed_span(
                ...
                metric_labels={"stage": "embed", "status": "success"},
            ):
            ...
            async with timed_span(
                ...
                await ledger.upsert(doc_id, source_path, content_hash, status="success")
            return _record_terminal("success", len(stored_chunks))
            ...
            return _record_terminal("failed", 0)
